...
--- config
    location /t {
        content_by_lua_block {
            local plugin = require("apisix.plugins.openid-connect-idp-selector")
...
=== TEST 3: A `configs` entry missing a required field is rejected.
--- config
    location /t {
        content_by_lua_block {
            local plugin = require("apisix.plugins.openid-connect-idp-selector")
...
=== TEST 4: rewrite() selects the config matching the bearer token's "iss" claim.
--- config
    location /t {
        content_by_lua_block {
...
=== TEST 5: rewrite() sets nothing when there is no bearer token, or when the token's issuer matches no config.
--- config
    location /t {
        content_by_lua_block {
            local jwt = require("resty.jwt")
...
=== TEST 6: End to end - route with both plugins, unresolved issuer fails closed with 500 from openid-connect.
--- config
    location /t {
        content_by_lua_block {
                 ...
                 [[{
                        "plugins": {
                            "openid-connect-idp-selector": {
                                "configs": [
                                    {
                                        "key": "http://127.0.0.1:8080/realms/University",
...
=== TEST 7: hitting the route from TEST 6 with no bearer token fails closed with 500.
--- config
    location /t {
        content_by_lua_block {
...
=== TEST 8: configs[].client_secret is encrypted at rest, not stored as the plaintext value set in TEST 6.
--- config
    location /t {
        content_by_lua_block {
            local etcd = require("apisix.core.etcd")
            local res = assert(etcd.get('/routes/oidc-selector-e2e'))
            local conf = res.body.node.value.plugins["openid-connect-idp-selector"]
