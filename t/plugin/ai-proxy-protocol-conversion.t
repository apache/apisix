...
=== TEST 29: SSE comment lines (starting with ':') must be ignored and not produce empty events
--- config
    location /t {
        content_by_lua_block {
            local sse = require("apisix.plugins.ai-transport.sse")

            -- A buffer containing SSE comment lines interleaved with real events.
            local buf = ": keep-alive\n\n" ..
                        "data: {\"id\":\"1\",\"choices\":[{\"delta\":{\"content\":\"hi\"}}]}\n\n" ..
                        ": heartbeat\n\n"

            local events, remainder = sse.decode_buf(buf)

            -- Only the real event should be decoded; comment-only blocks must be dropped.
            if #events ~= 1 then
                ngx.say("FAIL: expected 1 event, got " .. #events)
                return
            end

            local data = require("cjson.safe").decode(events[1].data)
            if not data or not data.choices or not data.choices[1].delta.content == "hi" then
                ngx.say("FAIL: unexpected event data: " .. events[1].data)
                return
            end

            if remainder ~= "" then
                ngx.say("FAIL: expected empty remainder, got: " .. remainder)
                return
            end

            ngx.say("OK: SSE comment lines correctly ignored")
        }
    }
--- response_body
OK: SSE comment lines correctly ignored
