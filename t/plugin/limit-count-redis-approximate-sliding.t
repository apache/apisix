...
=== TEST 3: redis policy with approximate sliding window - remaining header on reject
--- config
    location /t {
        content_by_lua_block {
...
=== TEST 4: redis policy with approximate sliding window - allow after window passes
--- config
    location /t {
        content_by_lua_block {
            local json = require "t.toolkit.json"
...
=== TEST 11: redis policy with approximate sliding window - enforce N per window under burst traffic
--- config
    location /t {
        content_by_lua_block {
            local t = require("lib.test_admin").test
...
=== TEST 12: redis policy with approximate sliding window - only N requests allowed in a burst
--- pipelined_requests eval
[
    "GET /burst",
    "GET /burst",
