...
=== TEST 14: modified redis script, cost == 2
--- config
    location /t {
        content_by_lua_block {
            ...
            res, err = red:eval(script, 1, uri, 3, 60, 2)
            if err then
                ngx.say("failed to limit count: ", err)
                return
            end

            ngx.say("remaining: ", res[1])
        }
    }
--- response_body
remaining: 1
...
=== TEST 15: modified redis script, cost < 1
--- config
    location /t {
        content_by_lua_block {
            local script = require("apisix.plugins.limit-count.limit-count-redis-cluster").script
            ...
            local uri = ngx.var.uri
            res, err = red:eval(script, 1, uri, 4, 60, 0)
            ngx.say(err)
        }
    }
--- response_body_like
