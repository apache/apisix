...
=== TEST 3: verify (in header) not hiding credentials, and check X-Consumer-Name should return
--- request
GET /echo
--- more_headers
...
=== TEST 4: data encryption for private_key
--- yaml_config
apisix:
    data_encryption:
        enable: true
...
=== TEST 5: set jwt-auth conf: secret uses secret ref
--- request
GET /t
--- config
    location /t {
...
=== TEST 6: store secret into vault
--- exec
...
=== TEST 7: verify (in header) not hiding credentials
--- request
GET /echo
--- more_headers
jwt-header: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJrZXkiOiJ1c2VyLWtleSIsImV4cCI6MTg3OTMxODU0MX0.fNtFJnNmJgzbiYmGB0Yjvm-l6A6M4jRV1l4mnVFSYjs
...
=== TEST 8: store rsa key pairs and secret into vault from local filesystem
--- exec
...
=== TEST 9: create consumer for RS256 algorithm with private/public key fetched from vault and public key in consumer schema
--- config
    location /t {
        content_by_lua_block {
            local t = require("lib.test_admin").test
...
=== TEST 10: sign a jwt with with rsa key pair and access /hello
--- config
    location /t {
        content_by_lua_block {
            local t = require("lib.test_admin").test
