...
=== TEST 1: Set up route with unauth_action = "pass" so unauthenticated requests pass through.
--- config
    location /t {
...
=== TEST 2: Access route without authentication. Client-supplied X-Userinfo / X-ID-Token / X-Refresh-Token / X-Access-Token must not reach upstream.
--- request
GET /uri HTTP/1.1
--- more_headers
...
=== TEST 4: Access route with valid bearer token and a client-supplied X-Access-Token. Upstream must see the validated token, not the client value.
--- request
GET /uri HTTP/1.1
--- more_headers
