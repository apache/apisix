...
=== TEST 6: add plugin with 'include_resp_body' setting
--- config
    location /t {
        content_by_lua_block {
            local t = require("lib.test_admin").test
            ...
            local code, body = t('/apisix/admin/routes/1',
                 ...
                 [[{
                        "plugins": {
                            "file-logger": {
                                "path": "file-with-resp-body-2.log",
                                "include_resp_body": true
                            }
                        },
                        "upstream": {
...
=== TEST 7: verify plugin for file-logger with response
--- config
    location /t {
        content_by_lua_block {
            local core = require("apisix.core")
            local t = require("lib.test_admin").test
            local code = t("/hello", ngx.HTTP_GET)
            local fd, err = io.open("file-with-resp-body-2.log", 'r')
            local msg
