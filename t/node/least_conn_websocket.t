...
=== TEST 4: test balanced initialization for new servers during scaling
--- apisix_yaml
upstreams:
  - id: 1
    type: least_conn
    ...
    location /test {
        content_by_lua_block {
            ...
            local upstream = {
                parent = {
                    value = {
                        id = "test_upstream_scaling"
                    }
                }
            }

            -- Simulate existing servers with high connection counts
            local ngx_shared = ngx.shared
            local conn_count_dict = ngx_shared["balancer-least-conn"]
            
            -- Set high connection counts for existing servers
            conn_count_dict:set("conn_count:test_upstream_scaling:127.0.0.1:1980", 1000)
            conn_count_dict:set("conn_count:test_upstream_scaling:127.0.0.1:1981", 1000)
            ...
            -- Create balancer with existing high-connection servers
            local balancer1 = balancer.new(up_nodes, upstream)

            -- Now add new servers (simulate scaling up)
            up_nodes["127.0.0.1:1982"] = 1
            up_nodes["127.0.0.1:1983"] = 1

            -- Create new balancer instance with scaled servers
            local balancer2 = balancer.new(up_nodes, upstream)

            -- Check that new servers got balanced initial connection counts
            local new_server1_count = conn_count_dict:get("conn_count:test_upstream_scaling:127.0.0.1:1982")
            local new_server2_count = conn_count_dict:get("conn_count:test_upstream_scaling:127.0.0.1:1983")

            ngx.say("new server 1 initial connections: ", new_server1_count or 0)
            ngx.say("new server 2 initial connections: ", new_server2_count or 0)
            ngx.say("balanced scaling completed successfully")
        }
    }
--- response_body
new server 1 initial connections: 800
new server 2 initial connections: 800
balanced scaling completed successfully
