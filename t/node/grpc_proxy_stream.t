...
=== TEST 2: Unary API gRPC proxy with tls
--- http2
--- tls
--- apisix_yaml
ssl:
...
--- config
    location /t {
        content_by_lua_block {
            local ngx_pipe = require("ngx.pipe")
            local proc, err = ngx_pipe.spawn("./build-cache/grpcurl -import-path ./build-cache/proto -proto helloworld.proto -insecure -d '{\"name\":\"apisix\"}' test.com:1984 helloworld.Greeter.SayHello")
            if not proc then
                ngx.say(err)
                return
            end
...
=== TEST 3: Unary API gRPC proxy test [the old way]
--- http2
--- apisix_yaml
routes:
  -
...
=== TEST 4: Unary API grpcs proxy test
--- http2
--- tls
--- apisix_yaml
ssl:
...
=== TEST 5: Unary API grpcs proxy test with mTLS
--- http2
--- https
--- apisix_yaml
ssl:
