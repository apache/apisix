...
./build-cache/grpcurl -plaintext -import-path ./t/grpc_server_example/proto -proto helloworld.proto -d '{"name":"apisix"}' 127.0.0.1:9081 helloworld.Greeter.SayHello | grep 'Hello apisix'

# test grpc proxy with ssl
./build-cache/grpcurl -insecure -import-path ./t/grpc_server_example/proto -proto helloworld.proto -d '{"name":"apisix"}' test.com:9443 helloworld.Greeter.SayHello | grep 'Hello apisix'
