...
def check_log():
    ...
    cmds = [
        'cat %s | grep -a "fail"' % boofuzz_log,
        'cat %s | grep -a "error" | grep -v "invalid request body"' % apisix_errorlog,
        'cat %s | grep -a " 500 "' % apisix_accesslog,
    ]
    for cmd in cmds:
    ...
    session = Session(
        target=Target(
            connection=TCPSocketConnection(
                "127.0.0.1", 9080, send_timeout=5.0, recv_timeout=5.0, server=False
            )
        ),
...
def run_test(create_route, run):
    ...
    if r2 != r1:
        print(
            "before test, nginx's process list:%s,\nafter test, nginx's process list:%s"
            % (r1, r2)
        )
