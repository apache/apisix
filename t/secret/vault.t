...
=== TEST 7: check key with the token in an env var: error format
--- config
    location /t {
        content_by_lua_block {
            local vault = require("apisix.secret.vault")
...
=== TEST 8: check key with the token in an env var: no main key
--- config
    location /t {
        content_by_lua_block {
            local vault = require("apisix.secret.vault")
...
=== TEST 9: check key with the token in an env var: no sub key
--- config
    location /t {
        content_by_lua_block {
            local vault = require("apisix.secret.vault")
...
=== TEST 10: error vault uri (with the token in an env var)
--- config
    location /t {
        content_by_lua_block {
            local vault = require("apisix.secret.vault")
...
=== TEST 11: get value from vault with the token in an env var
--- config
    location /t {
        content_by_lua_block {
            local vault = require("apisix.secret.vault")
