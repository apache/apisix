...
local function create_radixtree_router(routes)
    ...
    for _, route in ipairs(routes) do
        if type(route) == "table" then
            ...
            local uris = route.value.uris and core.table.clone(route.value.uris)
            uris = uris or {route.value.uri}
            for i, uri in ipairs(uris) do
                uris[i] = lower_str(uris[i])
            end
...
function _M.match(api_ctx)
    ...
    api_ctx.var.uri = lower_str(api_ctx.var.uri)
