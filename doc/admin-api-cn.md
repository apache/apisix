...
| service_protocol | 可选  | 上游协议类型 | 只可以是 "grpc", "http" 二选一。| 默认 "http"，使用 gRPC proxy 或 gRPC transcode 时，必须用"grpc" |
...
_地址_：/apisix/admin/consumers/{id}

_说明_：Consumer 是某类服务的消费者，需与用户认证体系配合才能使用。
