...
- `appid`: 设置应用 id, 该应用 id, 需要是在 `wolf-console` 中已经添加的应用 id.
- `header_prefix`: 自定义 http 头的前缀, 默认为: `X-`. `wolf-rbac`在鉴权成功后, 会在请求头(用于传给后端)及响应头(用于传给前端)中添加 3 个头: `X-UserId`, `X-Username`, `X-Nickname`.
...
![wolf-rbac-1](../images/plugin/wolf-rbac-1.png)

然后在 consumer 页面中添加 wolf-rbac 插件：

![wolf-rbac-2](../images/plugin/wolf-rbac-2.png)
