...
|resp_headers    |No| set a new to `headers` , and return it to client, can set up multiple. If upstream aleady return the same head field, will rewrite the header, otherwise will add the header. You can also set empty string to remove a header.|
