...
`ip-restriction` 可以通过以下方式限制对服务或路线的访问，将 IP 地址列入白名单或黑名单。 单个 IP 地址，多个 IP 地址 或 CIDR 范围，可以使用类似 10.10.10.0/24 的 CIDR 表示法(将很快支持 IPv6)。

## 属性

- `whitelist`: 可选，加入白名单的 IP 地址 或 CIDR 范围
- `blacklist`: 可选，加入黑名单的 IP 地址 或 CIDR 范围
