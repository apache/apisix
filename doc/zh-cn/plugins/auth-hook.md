...
`auth-hook` 是一个认证及授权(rbac)插件，它需要与 `consumer` 一起配合才能工作。同时需要添加 `auth-hook` 到一个 `service` 或 `route` 中。
...
| header_prefix | string | 可选   | "X-"                     |        | 自定义http头的前缀。`auth-hook`在鉴权成功后, 会在请求头(用于传给后端)及响应头(用于传给前端)中添加3个头: `X-UserId`, `X-Username`, `X-Nickname` |
