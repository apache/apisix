...
注意：在启用 `Admin API` 时，它会占用前缀为 `/apisix/admin` 的 API。 因此，为了避免您设计 API 与 `/apisix/admin` 冲突，建议数据面与控制面的端口保持不同, 您可以在 `conf/config.yaml` 中通过 `port_admin` 进行自定义 Admin API 端口。
