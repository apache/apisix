...
|nodes           |必需|哈希表，内部元素的 key 是上游机器地址列表（IP+Port 方式），value 则是节点的 weight。对于 `roundrobin` 算法，节点的选择是依赖这个权重值的，但对于 `chash` 算法，这个权重值的意义不大。特别的，当权重值为 `0` 有特殊含义，表示该上游节点是无效的，将不会参与上游节点选择。|
