...
|key             |必需|该选项只有类型是 `chash` 才有效。根据 `key` 来查找对应的 node `id`，相同的 `key` 在同一个对象中，永远返回相同 id,目前支持nginx内置变量有`uri,server_name,server_addr,request_uri,remote_port,remote_addr,query_string,host,hostname,arg_xxx`，其中`arg_xxx` 是来自URL的请求参数，[内置变量说明](http://nginx.org/en/docs/varindex.html)|
