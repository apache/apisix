...
init_kv["/apisix/consumers/"]="init_dir"
init_kv["/apisix/global_rules/"]="init_dir"
init_kv["/apisix/node_status/"]="init_dir"
init_kv["/apisix/plugin_metadata/"]="init_dir"
init_kv["/apisix/plugins/"]="init_dir"
init_kv["/apisix/proto/"]="init_dir"
init_kv["/apisix/routes/"]="init_dir"
init_kv["/apisix/services/"]="init_dir"
init_kv["/apisix/ssl/"]="init_dir"
init_kv["/apisix/stream_routes/"]="init_dir"
init_kv["/apisix/upstreams/"]="init_dir"
words=($cmd_res)

for ((i=0;i<${#words[@]};i++))
do
if [ "${init_kv[${words[$i]}]}" == "" ]; then
    let i=$i+1
    continue
fi
if [ "${init_kv[${words[$i]}]}" != "${words[$i+1]}" ]; then
    echo "failed: index=$i, ${init_kv[words[$i]]} is not equal to ${words[$i+1]}"
    exit 1
fi
let i=$i+1
done
