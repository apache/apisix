...
create_lua_deps() {
    WITHOUT_DASHBOARD=1 sudo luarocks make --lua-dir=${OPENRESTY_PREFIX}/luajit rockspec/apisix-master.rockspec --tree=deps --only-deps --local
    ...
    sudo cp rockspec/apisix-master.rockspec build-cache/
}
...
do_install() {
    ...
    if [ ! -f "build-cache/apisix-master.rockspec" ]; then
        create_lua_deps

    else
        src=`md5sum rockspec/apisix-master.rockspec | awk '{print $1}'`
        src_cp=`md5sum build-cache/apisix-master.rockspec | awk '{print $1}'`
        if [ "$src" = "$src_cp" ]; then
