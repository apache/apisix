...
| metadata     | object | optional    |         |       | Filter service instances by metadata using string equality matching. Changes take effect after the next upstream node fetch. |
...
APISIX supports filtering service instances based on metadata. When a route is configured with metadata conditions, only service instances whose metadata contains all the key-value pairs specified in the route's `metadata` configuration will be selected. Each metadata value must be a string, and a service instance matches when its metadata value equals the configured string.
...
This route will only route traffic to service instances that have `env: "prod"` and `lane: "a"` in their metadata.
