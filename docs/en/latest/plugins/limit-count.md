...
| rules                   | array[object]     | False                                     |               |                            | A list of rate limiting rules. Each rule is an object containing `count`, `time_window`, and `key`, and every rule must use a unique `key`. Cannot be configured together with the top-level `count`, `time_window`, or `group`. |
