...
| rules.count             | integer or string | True                                      |               | > 0                        | Maximum requests allowed within the rule's `time_window`. A string variable must resolve to a positive integer no greater than `9007199254740991`; otherwise APISIX returns `500 Internal Server Error` unless `allow_degradation` is `true`. |
| rules.time_window       | integer or string | True                                      |               | > 0                        | Time interval for the rule's `count`, in seconds. A string variable must resolve to a positive integer no greater than `9007199254740991`; otherwise APISIX returns `500 Internal Server Error` unless `allow_degradation` is `true`. |
