...
The LLM call used for rewriting is separate from the client's request format. With the `openai` provider, the Plugin builds a non-streaming Chat Completions request from the configured prompt and the original request body. It does not classify or proxy the client's request as an AI protocol.
