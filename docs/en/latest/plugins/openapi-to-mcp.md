...
| max_expanded_nodes | integer | False | `50000` | >= 1000 | Maximum number of nodes one `$ref` expansion may produce in the tool input schemas. Past it the rest of that expansion degrades to a generic object, which a very large document can reach legitimately. Only `paths` is expanded, so the document's other sections do not count towards it. |
...
| allowed_origins | array[string] | False | | | `Origin` header values accepted on MCP requests, written as `scheme://host[:port]`. When set, the list governs on its own; `["*"]` accepts any origin. When unset, an `Origin` is accepted only if its host is a literal host the Route declares in `host`/`hosts` -- a `*.example.com` entry does not count -- or if both it and the request are on a loopback address. A request with no `Origin` header is always accepted. |
...
* **A request carrying an `Origin` is refused unless something the operator configured says otherwise.** MCP requires an HTTP transport to validate `Origin`, because a page in a browser can otherwise reach a server that only listens on localhost, or one behind the user's firewall, and read the answer back. The `Origin` is never compared against another header of the same request: under DNS rebinding the attacker owns the name, so `Origin` and `Host` are both theirs and agree with each other. It is checked against `allowed_origins`, or against the literal hosts the Route declares in `host`/`hosts`, or -- where the Route declares neither -- accepted only when both the origin and the request are on a loopback address, which an attacker's page cannot be. A request with no `Origin` is untouched: no non-browser MCP client sends one, and those are most of them. A `*.example.com` entry in `host`/`hosts` is a routing predicate, not an origin allowlist -- whoever controls one name under it can serve a page there -- so a Route matched on a wildcard accepts an origin only through `allowed_origins`.
