...
Missing or null stream metadata is ignored when constructing the result, using previously received valid values or generated defaults. Original upstream events are not repaired; invalid original metadata can still affect SDK parsing. Clean EOF without an upstream finish reason does not acquire a synthetic `stop` reason.
