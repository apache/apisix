...
The following example demonstrates the connection between the `response-rewrite` Plugin and [execution phases](../terminology/plugin.md#plugins-execution-lifecycle) by configuring the Plugin with the `key-auth` Plugin, and see how the response is still rewritten to `200 OK` in the case of an unauthenticated request.
