...
| cache_set_cookie   | boolean        | False    | false                     |                         | If true, cache responses that include a `Set-Cookie` header. Off by default because `Set-Cookie` is per-recipient and not safe for a shared cache to store. Enable this only for routes where the upstream's `Set-Cookie` is not user-specific (for example, an A/B-testing variant cookie). |
