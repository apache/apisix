...
| cache_control      | boolean        | optional    | false                     |                                                                                 | Whether to comply with the cache-control behavior in the HTTP specification. |
...
| cache_ttl          | integer        | optional    | 300 seconds               |                                                                                 | The default cache time. when the cache_control option is not enabled or the proxied server does not return cache header (cache-contorl or expires), it will be take effect. |
