...
| cache.ttl         | integer       | False    | 5       | between 1 and 3600 inclusive | How long, in seconds, a decision is cached. A shorter TTL bounds how long a stale decision can be served. If the authorization response carries `Cache-Control: s-maxage=<n>` (or `max-age=<n>` when `s-maxage` is absent), the effective TTL is capped at `<n>`.                          |
...
Caching is fully opt-in; with no `cache` block the Plugin calls the authorization service on every request as before. The cache key always includes the request scheme, host, and client IP (all of which are forwarded to the authorization service as `X-Forwarded-*`), so a decision is never reused across a different origin or client address. Responses carrying `Cache-Control: no-store`, `no-cache`, or `private` are never cached, and transient `5xx` responses from the authorization service are never cached. When `request_method` is `POST`, the forwarded request body is also part of the cache key. Only enable caching when the decision is a stable function of the configured `cache.key_headers` (plus method/URI/body where applicable).
