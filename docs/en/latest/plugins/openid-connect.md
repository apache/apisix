...
| proxy_opts.https_proxy_authorization | string   | False    |     | Basic [base64 username:password] | Default `Proxy-Authorization` header value to be used with `https_proxy`. Cannot be overriden with custom `Proxy-Authorization` request header since with HTTPS, the authorization is completed when connecting.               |
