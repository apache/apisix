...
| session.redis.mode | string | False | | ["storage", "revocation"] | Role of this Redis connection. Use `storage` when `session.storage` is `redis`; use `revocation` only when `session.storage` is `cookie`. Defaults to `storage` or `revocation` respectively when omitted. Combining `storage=redis` with `mode=revocation` is rejected. |
| session.revocation_fail_mode | string | False | open | ["open", "closed"] | When the revocation store is unreachable: `open` treats the cookie session as not revoked; `closed` rejects session open/destroy. Only applies when cookie-session revocation is enabled. |
