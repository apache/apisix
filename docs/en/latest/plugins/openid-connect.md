...
| introspection_with_scope             | boolean | False    | false                 |              | When set to `true`, APISIX will include `scope` which configured along with route configuration to verify with instrospection endpoint to verify access's token scope against granted scope . Note this only support by some oauth server. Example Ory Hydra. For other server, see `introspection_verify_scope`. |
| introspection_verify_scope           | boolean | False    | false                 |              | When set to `true`, APISIX will compare `scope` which configured along with route configuration with scope responded from introspection endpoint. If instrospection endpoint does not return scope in response, verification will not happen.                                                                     |
