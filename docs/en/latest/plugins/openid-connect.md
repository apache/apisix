...
| set_id_token_original_header         | boolean  | False    | true                  |              | If true and if the ID token is available, set the value in the `X-ID-Token-Original` request header. 
|                                      |          |          |                       |              | This header contains the original ID-Token with JWS signature and Headers (in contrast to the `X-ID-Token` header). |
