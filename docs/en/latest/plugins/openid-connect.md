...
| introspect_session_access_token | boolean | False | false | | If true, introspect the access token held in the session on every request, so that a token revoked at the identity provider stops being accepted. Cannot be enabled together with `bearer_only`. Requires an introspection endpoint, either set in `introspection_endpoint` or published by the discovery document. When the token is reported inactive, the session is discarded and the request follows `unauth_action`; the negative verdict is cached for 10 seconds. When no verdict can be obtained, the request fails with `503` and the session is kept. Active verdicts are cached only when `introspection_interval` is greater than 0, delaying revocation detection by up to that TTL. |
