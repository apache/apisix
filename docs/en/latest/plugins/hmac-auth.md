...
:::note
The `validate_request_body` option ensures that the `Digest` header matches the request body, but it does not automatically include the `Digest` header in the HMAC signature. To bind the request body to the signature, you should include the `Digest` header in the signed headers list, as shown in the example below.
:::
