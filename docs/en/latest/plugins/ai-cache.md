...
| redis_cluster_nodes        | array[string]   | True when `policy` is `redis-cluster` |     |                      | List of Redis cluster node addresses (at least two), used when `policy` is `redis-cluster`.          |
