...
| rules.header_prefix          | string         | False    |          |                                                         | Prefix for rate limit headers. If configured, the response will include `X-AI-{header_prefix}-RateLimit-Limit`, `X-AI-{header_prefix}-RateLimit-Remaining`, and `X-AI-{header_prefix}-RateLimit-Reset` headers. If not configured, the rule index (1-based) will be used as the prefix. |
