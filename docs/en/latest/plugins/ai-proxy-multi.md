...
| instances.override.path_mode        | string         | False    | fixed                             | fixed, preserve, append | Controls how the upstream request path is determined per instance. `fixed` uses the endpoint path or the provider default. `preserve` uses the original request URI path. `append` appends the original request URI path to the endpoint path. When `instances.override.endpoint` is not set, `preserve` and `append` both fall back to the original request URI path. |
