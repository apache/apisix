...
| instances.provider_conf.region     | string         | True     |                                   | minLength = 1 | AWS region used to construct the Bedrock endpoint and to sign the request with SigV4. Used when `provider` is `bedrock`. If not set, falls back to the `AWS_REGION` environment variable, then defaults to `us-east-1`. |
...
| instances.auth.aws                  | object         | False    |                                   |              | Configuration for AWS authentication. Required when `provider` is `bedrock`. |
| instances.auth.aws.access_key_id    | string         | True     |                                   | minLength = 1 | AWS access key ID used for SigV4 signing. |
| instances.auth.aws.secret_access_key | string        | True     |                                   | minLength = 1 | AWS secret access key used for SigV4 signing. Stored encrypted. |
| instances.auth.aws.session_token    | string         | False    |                                   | minLength = 1 | Optional AWS session token for temporary credentials (e.g., from STS or assumed roles). Stored encrypted. |
