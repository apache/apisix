...
| instances.provider                 | string         | True     |                                   | [openai, deepseek, azure-openai, aimlapi, gemini-openai, openai-compatible] | LLM service provider. When set to `openai`, the Plugin will proxy the request to `api.openai.com`. When set to `deepseek`, the Plugin will proxy the request to `api.deepseek.com`. When set to `aimlapi`, the Plugin uses the OpenAI-compatible driver and proxies the request to `api.aimlapi.com` by default. When set to `gemini-openai`, the Plugin uses the OpenAI-compatible driver and proxies the request to `generativelanguage.googleapis.com/v1beta/openai/chat/completions` by default. When set to `openai-compatible`, the Plugin will proxy the request to the custom endpoint configured in `override`. |
