...
| slow_start.slow_start_time_seconds | True, when `slow_start` is set | Integer | Slow start window in seconds. A node the gateway observes for the first time takes a reduced share of the traffic and ramps back to its configured weight over this window. Must be at least 1. | 300 |
| slow_start.min_weight_percent | True, when `slow_start` is set | Integer | Lowest effective weight during the ramp, as a percentage of the configured weight, from 1 to 100. | 1 |
| slow_start.interval | False | Integer | Seconds between two effective weight refreshes. Defaults to `1`, and cannot be greater than `slow_start_time_seconds`. | 1 |
| slow_start.aggression | False | Number | Shape of the ramp. `1` (default) is linear, above `1` ramps up faster at the beginning, below `1` slower. At least `0.01`. | 1 |
| slow_start.default_weight | False | Integer | The weight the nodes of a Kubernetes Upstream ramp up to, since its endpoints carry no weight of their own. Only supported when `discovery_type` is `kubernetes`. | 100 |
| slow_start.startup_grace_period_seconds | False | Integer | Seconds after the gateway starts during which a node observed for the first time is treated as already warmed up, so that a restart does not ramp the whole node set again. Defaults to `0`. | 180 |
...
`slow_start` ramps the nodes of a `roundrobin` Upstream up to their configured weight. Whether a node is new is decided by the gateway itself, from the node set it observes, and the start of each ramp is recorded locally in the `upstream-slow-start` shared dict:

- The node set an Upstream has when the gateway first builds a load balancer for it is treated as warmed up. So is the node set it already has when `slow_start` is turned on.
...
- A node is identified by the workload behind it where service discovery knows one: a Kubernetes endpoint by the Pod, so an address a different Pod takes over ramps again, while a Pod that changes address keeps the progress it had. Everything else is identified by address, and a domain node by its hostname.
