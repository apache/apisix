...
| pass_host      | 可选                               | 枚举           | 指定上游请求 host 的来源，可以是 [`pass`，`node`，`rewrite`] 之一。`pass`: 使用客户端请求的 host 作为上游请求的 host, `node`: 使用 `upstream` 的 node 中配置的 host 作为上游请求 host, `rewrite`: 使用 `upstream_host` 配置的值重写上游请求 host。默认为 `pass` 方式。                                                                                                                                                                        |                                                  |
