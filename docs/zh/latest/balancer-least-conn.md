...
每个上游服务器被分配一个动态得分，该得分结合了其基础权重和当前连接负载：
...
其中：
- `base_score = 1 / weight` - 与服务器权重成反比的基础得分
- `connection_count` - 当前到服务器的活跃连接数
- `weight_factor = 1 / weight` - 权重调整因子

得分较低的服务器优先获得新连接。得分会随着连接的建立和完成实时更新。

### 连接状态管理

#### 实时更新
- **连接开始**：得分增加 `1/weight`，连接计数递增
- **连接结束**：得分减少 `1/weight`，连接计数递减
