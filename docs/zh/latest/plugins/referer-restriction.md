...
`referer-restriction` 插件允许用户将 `Referer` 请求头列入白名单或黑名单来限制对服务或路由的访问。
...
| bypass_missing  | boolean       | 否    | false   |       | 当 `Referer` 请求头不存在或格式有误时，是否绕过检查。如果设置该选项为 `true`，那么将绕过检查。 |
...
通过上述命令启用插件后，你可以将请求设置为 `Referer: http://xx.com/x` 来测试插件：
...
因为启用插件时将属性 `bypass_missing` 设置为 `true`，所以未指定 `Refer` 请求头的请求将跳过检查：
...
当你需要禁用 `referer-restriction` 插件时，可以通过以下命令删除相应的 JSON 配置，APISIX 将会自动重新加载相关配置，无需重启服务：
