...
| bypass_malformed | boolean       | 否   | true                               |           | 当设置为 `false` 时，如果 `Referer` 请求头格式有误，将阻断请求；当设置为其它值时，如果 `Referer` 请求头格式有误，将按照 `Referer` 请求头不存在处理，参考配置项 `bypass_missing`。 |
| bypass_missing   | boolean       | 否   | false                              |           | 当设置为 `true` 时，如果 `Referer` 请求头不存在，将绕过检查。                                                                               |
