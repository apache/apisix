...
`match` 是用于引导流量的自定义规则，`weighted_upstreams` 是用于引导流量的一组上游服务。当一个请求被 `match` 属性匹配时，它将根据配置的 `weights` 属性被引导至上游服务。你也可以不使用 `match` 属性，而根据 `weighted_upstreams` 属性引导所有流量。
...
| rules.match.vars               | array[array]  | 否   |        |        | 由一个或多个 `{var, operator, val}` 元素组成的列表，例如：`{"arg_name", "==", "json"}`，表示当前请求参数 `name` 是 `json`。这里的 `var` 与 NGINX 内部自身变量命名是保持一致，所以也可以使用 `request_uri`、`host` 等；对于已支持的运算符，具体用法请看 [lua-resty-expr](https://github.com/api7/lua-resty-expr#operator-list) 的 `operator-list` 部分。 |
...
| upstream.type                  | enum   | 否   |   roundrobin |  [roundrobin, chash]      | 流量引导机制的类型，`roundrobin` 支持权重的负载，`chash` 使用一致性哈希。                                                                                                                                                                                                                                                                                                                           |
...
| upstream.timeout               | object | 否   |  15     |        | 设置连接、发送消息、接收消息的超时时间（单位为秒）。                                                                                                                                                                                                                                                                                                                           |
