...
| recursive         | boolean       | 可选  | true或者false                                         | 如果禁用递归搜索，则与受信任地址之一匹配的原始客户端地址将替换为由 real_ip_header 指令定义的请求标头字段中发送的最后一个地址。如果启用递归搜索，则与其中一个受信任地址匹配的原始客户端地址将替换为请求标头字段中发送的最后一个非受信任地址。 |
