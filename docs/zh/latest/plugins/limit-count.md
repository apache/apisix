...
| rules.header_prefix | string | 否 | | | 速率限制头部的前缀。如果配置了此项，响应将包含 `X-{header_prefix}-RateLimit-Limit`、`X-{header_prefix}-RateLimit-Remaining` 和 `X-{header_prefix}-RateLimit-Reset` 头部。如果未配置，将使用规则索引（从 1 开始）作为前缀。|
