...
- 当 `window_type` 为 `approximate_sliding` 且 `policy` 为 `redis` 或 `redis-cluster` 时，插件使用基于当前和前一个固定窗口加权计数的轻量级**近似滑动窗口**算法。相比精确滑动窗口，该算法在准确性和性能之间取得良好平衡，内存占用更低。
...
| window_type | string | 否 | fixed | ["fixed","sliding","approximate_sliding"] | 窗口行为类型。`fixed` 使用固定窗口算法；`sliding` 使用滑动窗口算法，在滚动时间窗口内精确限制请求次数；`approximate_sliding` 使用基于当前和前一个固定窗口加权计数的轻量级近似滑动窗口算法。`sliding` 和 `approximate_sliding` 仅当 `policy` 为 `redis` 或 `redis-cluster` 时才支持。 |
