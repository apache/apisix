...
| max_req_body_bytes     | integer | 否     | 524288            | [1,...]               | 当` include_req_body `属性设置为` true `时生效，限制请求体记录的最大字节数，超过则中断。** 3.10版本开始支持。** |
| max_resp_body_bytes    | integer | 否     | 524288            | [1,...]               | 当` include_resp_body `属性设置为` true `时生效，限制响应体记录的最大字节数，超过则中断。** 3.10版本开始支持。** |
