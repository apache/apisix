...
| api_version           | integer | 否     | 1              | [0, 1, 2]             | [lua-resty-kafka](https://github.com/doujiang24/lua-resty-kafka) 中的 Produce API 版本。默认 `1`，与上游库保持一致。使用 Apache Kafka 4.x 时需设置为 `2`（Kafka 4.x 弃用了 magic0 和 magic1）。 |
