...
| producer_batch_num | integer | 可选    | 200 | [1,...] | 对应[lua-resty-kafka]中的`batch_num`参数,聚合消息批量提交,单位为消息条数 |
| producer_batch_size | integer | 可选    | 1048576 | [0,...] | 对应[lua-resty-kafka]中的`batch_size`参数,单位为字节 |
| producer_max_buffering | integer | 可选    | 50000 | [1,...] | 对应[lua-resty-kafka]中的`max_buffering`参数,最大缓冲区,单位为条 |
| producer_time_linger | integer | 可选    | 1 | [1,...] | 对应[lua-resty-kafka]中的`flush_time`参数,单位为秒 |
