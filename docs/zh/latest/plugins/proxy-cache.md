...
| cache_set_cookie | boolean | 否 | false | | 如果为 true，缓存包含 `Set-Cookie` 响应头的响应。默认关闭，因为 `Set-Cookie` 是面向特定接收方的，不适合放入共享缓存。仅当上游的 `Set-Cookie` 与具体用户无关（例如 A/B 测试变体 cookie）时才启用。 |
