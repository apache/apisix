...
当响应为流式（`stream: true`）时，插件会**缓冲完整的 SSE 响应，对组装后的补全内容扫描一次，然后才将其释放**给客户端。这是实现拦截所必需的：被标记的部分 token 绝不能到达客户端。通过扫描的响应会以其原始 SSE 帧格式原样转发；被标记的响应会被替换为以 `data: [DONE]` 结尾的、与提供商兼容的拒绝 SSE。

:::note

由于响应会被缓冲，流式输出扫描以牺牲逐 token 的增量传输来换取拦截能力——客户端在扫描完成后一次性收到响应，而不是逐 token 接收。由于缓冲开始时流的 `200`/`text/event-stream` 响应头已经提交，无论 `deny_code` 为何值，流式拦截始终以拒绝 SSE 响应体的形式发出。如果上游异常结束流而没有终止事件（例如连接中断），缓冲的内容将不会被释放。
