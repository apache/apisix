...
| cost_expr | string | False | | | 用于动态计算令牌消耗的 Lua 算术表达式。变量从 LLM API 原始使用量响应字段注入。嵌套字段以叶子字段名暴露，例如 `input_tokens_details.cached_tokens` 可直接用 `cached_tokens` 引用。字段名冲突时，层级较浅的字段优先。缺失的变量默认为 0。仅在 `limit_strategy` 为 `expression` 时有效。示例：`input_tokens + cache_creation_input_tokens + output_tokens`。 |
