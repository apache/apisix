...
| instances.provider_conf.project_id | string         | 是     |                                   |              | Google Cloud 项目 ID。当 `provider` 设置为 `vertex-ai` 时必填。 |
| instances.provider_conf.region     | string         | 是     |                                   |              | Google Cloud 区域。当 `provider` 设置为 `vertex-ai` 时必填。 |
...
| instances.auth.gcp.max_ttl          | integer        | 否    |                                   | minimum = 1  | 缓存 GCP 访问令牌的最大 TTL（秒）。 |
