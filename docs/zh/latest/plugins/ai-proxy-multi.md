...
| instances.override.endpoint         | string         | 否    |                                   |              | 用于替换默认端点的 LLM 提供商端点。如果未配置，插件使用默认的 OpenAI 端点 `https://api.openai.com/v1/chat/completions`。覆盖 URL 可以包含路径（和查询字符串）；当提供路径时，将覆盖插件原本计算的路径。对于 Bedrock，如果覆盖 URL 包含含有保留字符的路径（例如推理配置文件 ARN 中的 `:` 或 `/`），这些字符必须进行 URL 编码（`:` → `%3A`，`/` → `%2F`），以确保模型 ID 被保留为单个路径段。 |
