...
| override.endpoint | string | 否    |         |                                          | 自定义 LLM 提供商端点，当 `provider` 为 `openai-compatible` 时必需。当 `provider` 为 `bedrock` 时，可以设置为自定义的 Bedrock 端点。如果路径包含保留字符（例如推理配置文件 ARN 中的 `:` 和 `/`），必须分别 URL 编码为 `%3A` 和 `%2F`。 |
