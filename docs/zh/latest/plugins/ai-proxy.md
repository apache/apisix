...
| provider_conf.region | string | 否  |         | minLength = 1                            | 用于构造 Bedrock 端点并使用 SigV4 对请求进行签名的 AWS 区域。当 `provider` 为 `bedrock` 时使用。如未设置，则回退到 `AWS_REGION` 环境变量，否则默认为 `us-east-1`。 |
