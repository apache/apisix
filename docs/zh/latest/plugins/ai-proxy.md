...
| override.request_body | object | 否  |         |                                          | 请求体覆盖配置。支持 `max_tokens`（整数）字段，用于设置最大输出 token 数。APISIX 会根据不同的上游服务商自动映射到对应的字段名（例如 OpenAI 使用 `max_completion_tokens`，Responses API 使用 `max_output_tokens`，其他大多数服务商使用 `max_tokens`）。默认情况下客户端请求参数优先，override 仅补充缺失字段。设置 `request_body_force_override` 为 `true` 可让 override 强制覆盖客户端参数。 |
