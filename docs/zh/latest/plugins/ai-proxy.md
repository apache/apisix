...
| override.request_body | object | 否  |         |                                          | 按目标协议（`openai-chat`、`openai-responses`、`openai-embeddings`、`anthropic-messages`）配置的请求体覆盖。每个 key 对应一份部分请求体，会在协议转换后以**深度合并**的方式注入到发给上游的请求体中。适用于参数名因协议而异的场景（例如 `max_tokens` vs `max_output_tokens`）。对象递归合并，数组与标量整体替换。在 `options` 之后应用，同名字段以 `request_body` 为准。 |
