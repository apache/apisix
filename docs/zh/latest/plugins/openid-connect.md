...
| session.cookie.lifetime              | object   | False    | 3600                  |             | 用于设置 cookie 的生命周期，以秒为单位。默认设置为 3600 秒。这可以通过 Nginx `set $session_cookie_lifetime 3600` 来配置。在默认情况下不会影响会话 cookie 的过期时间，但如果通过 session.cookie.persistent == true 配置了持久化的 cookie，则会使用该值。另请参阅有关 ssl_session_timeout 的说明。                                                                                                               |
