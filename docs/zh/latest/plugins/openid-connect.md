...
| session | object | 否 | | | 当 `bearer_only` 为 `false` 且插件使用 Authorization Code 流程时使用的 Session 配置。这些选项直接映射到 [lua-resty-session](https://github.com/bungle/lua-resty-session#configuration) 4.x 配置。 |
| session.secret | string | 是 | | 16 个字符以上 | 当 `bearer_only` 为 `false` 时，用于 session 加密和 HMAC 运算的密钥。|
| session.cookie_name | string | 否 | session | | 会话 Cookie 名称。 |
| session.cookie_prefix | string | 否 | | | Cookie 前缀，例如 `__Host-` 或 `__Secure-`。 |
| session.cookie_path | string | 否 | / | | Cookie 路径范围。 |
| session.cookie_domain | string | 否 | | | Cookie 域范围。 |
| session.cookie_http_only | boolean | 否 | true | | 限制 Cookie 仅 HTTP 访问（禁止 JavaScript 访问）。 |
| session.cookie_secure | boolean | 否 | | | 仅通过 HTTPS 传输 Cookie。 |
| session.cookie_priority | string | 否 | | ["Low", "Medium", "High"] | Cookie 优先级。 |
| session.cookie_same_site | string | 否 | Lax | ["Lax", "Strict", "None", "Default"] | Cookie SameSite 策略。 |
| session.cookie_same_party | boolean | 否 | | | 启用 SameParty Cookie 标志。 |
| session.cookie_partitioned | boolean | 否 | | | 启用分区 Cookie（CHIPS）。 |
| session.idling_timeout | integer | 否 | 900 | | 会话空闲超时时间（秒）。如果会话在此时间内无活动，则会话将失效。 |
| session.rolling_timeout | integer | 否 | 3600 | | 会话滚动超时时间（秒）。超过此时间后会话将被强制续期。 |
| session.absolute_timeout | integer | 否 | 86400 | | 会话绝对超时时间（秒）。无论活动状态如何，超过此时间后会话将被销毁。 |
| session.remember | boolean | 否 | false | | 启用持久化会话（记住我）。 |
| session.remember_cookie_name | string | 否 | remember | | 持久化会话 Cookie 名称。 |
| session.remember_rolling_timeout | integer | 否 | 604800 | | 持久化会话滚动超时时间（秒）。 |
| session.remember_absolute_timeout | integer | 否 | 2592000 | | 持久化会话绝对超时时间（秒）。 |
| session.remember_safety | string | 否 | Medium | ["None", "Low", "Medium", "High", "Very High"] | 持久化会话的密钥派生复杂度。 |
| session.audience | string | 否 | default | | 会话受众（应用标识符）。 |
| session.subject | string | 否 | | | 会话主体（用户标识符）。 |
| session.enforce_same_subject | boolean | 否 | false | | 跨受众强制匹配主体。 |
| session.stale_ttl | integer | 否 | 10 | | 续期后旧会话的存活时间（秒）。 |
| session.touch_threshold | integer | 否 | 60 | | 会话更新的最小间隔时间（秒）。 |
| session.compression_threshold | integer | 否 | 1024 | | 触发压缩的最小负载大小（字节）。 |
| session.hash_storage_key | boolean | 否 | false | | 使用 SHA-256 哈希存储键。 |
| session.hash_subject | boolean | 否 | false | | 哈希主体以保护个人身份信息。 |
| session.store_metadata | boolean | 否 | false | | 持久化会话元数据（受众和主体）。 |
