...
首先，使用 Admin API 发送一个 `PUT` 请求，将鉴权模型和鉴权策略配置信息添加到插件的元数据中，通过这种方式创建路由的同时会根据请求中的配置创建一个 Casbin enforcer，并绑定到该路由。同时也可以使用 `PUT` 请求更新鉴权模型和鉴权策略配置信息，该插件将会自动同步最新的配置信息。
