...
| clickhouse.password                     | string  | 是     |                                |                                                                                         | ClickHouse 密码。当 `clickhouse` 存在时为必填。密码在存入 etcd 前会经过 AES 加密。详见[加密存储字段](../plugin-develop.md#加密存储字段)。                     |
...
本插件支持使用批处理器来聚合并批量处理条目（日志/数据），避免频繁提交数据。默认情况下，批处理器每 `3` 秒或队列中数据达到 `1000` 条时提交一次。详情及自定义配置请参考[批处理器](../batch-processor.md#配置)。
