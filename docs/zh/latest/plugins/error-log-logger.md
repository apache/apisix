...
| -------------------------------- | ------- | ----- | ------------------------------ | ---------------- | -------------------------------------------------------------------------|
| type                             | string  | 可选   | TCP                            | [TCP,SKYWALKING] | 接收器的类型                                                               |
...
| host                             | string  | 可选   |                                |                  | (`弃用`) TCP 服务的IP地址或主机名                                           |
| port                             | integer | 可选   |                                | [0,...]          | (`弃用`) Target upstream port                                            |
| tls                              | boolean | 可选   | false                          |                  | (`弃用`) 用于控制是否执行SSL验证                                             |
| tls_server_name                  | string  | 可选   |                                |                  | (`弃用`) TLS 服务名称标记                                                  |
