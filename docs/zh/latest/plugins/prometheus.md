...
| ------------ | ------ | ------ | -------- | ------------ | ----------------------------------------------------- |
| prefer_type   | string | 可选    | "id"     | ["id", "name"] | Prometheus 指标中，路由和服务的表现形式。          |
...
`prometheus` 插件可以使用空 {} 开启。一个可选的选项为 `prefer_type`，其默认值为 `id`。
当设置为 `name` 时，Prometheus指标中路由和服务的表现形式会从其 id 改为名称。
注意，多个路由/服务可以设置为相同的名称，因此注意规范命名，否则容易引起误解。
