...
### `apisix_ai_cache_hits_total` 的标签

`apisix_ai_cache_hits_total` 统计由 [`ai-cache`](./ai-cache.md) 插件缓存命中并直接返回的请求数，按命中的缓存层区分。

| 名称 | 描述 |
| ---------- | ----------------------------------------------------------------------------------------------------------------------------- |
| layer      | 命中的缓存层：`exact` 或 `semantic`。                                                                                 |
| route_id      | 指标对应的路由 ID。如果请求不匹配任何路由，则默认为空字符串。                         |
| service_id    | 匹配路由所属的服务 ID。如果匹配的路由不属于任何服务，则默认为空字符串。 |
| consumer   | 与请求关联的消费者名称。如果请求没有与之关联的消费者，则默认为空字符串。                       |
| node       | 上游节点的 IP 地址。缓存命中的请求不会到达上游，此标签为空字符串。                                                                                          |
| request_type       | traditional_http / ai_chat / ai_stream                                                                                          |
| request_llm_model       | 客户端请求的模型名称。                                                                                          |
| llm_model       | LLM 响应中报告的模型名称。缓存命中的请求不会到达上游，此标签为空字符串。                                                                                          |

### `apisix_ai_cache_misses_total` 的标签

`apisix_ai_cache_misses_total` 统计经过 [`ai-cache`](./ai-cache.md) 插件查询但未命中缓存的请求数。
...
### `apisix_ai_cache_bypasses_total` 的标签

`apisix_ai_cache_bypasses_total` 统计完全绕过 [`ai-cache`](./ai-cache.md) 插件缓存查询的请求数。
...
### `apisix_ai_cache_embedding_latency` 的标签

`apisix_ai_cache_embedding_latency` 是 [`ai-cache`](./ai-cache.md) 插件语义层发起的 embedding 调用延迟（毫秒）的直方图，围绕 embedding 服务的完整往返计时，成功与失败的调用均会记录。
