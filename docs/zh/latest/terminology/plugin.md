...
自 APISIX 3.17.0 起，对于匹配 HTTP Route 的请求，全局规则插件和局部绑定的插件会按阶段交错执行：
...
在 APISIX 3.16.0 及更早版本中，全局规则插件的两个阶段都先于局部绑定插件的阶段执行。按照当前顺序，局部绑定插件的 `rewrite` 处理函数会先于所有全局规则插件的 `access` 处理函数执行。如果某个 `rewrite` 处理函数终止了请求，则排在它之后的 `access` 处理函数不会执行，但 `log` 处理函数仍会执行。
