...
在每个阶段内，你可以在插件的 `_meta.priority` 字段中可选地定义一个新的优先级数。在执行期间，该优先级数优先于默认插件优先级。优先级数更大的插件先执行。

例如，如果你想在请求到达路由时，让 `limit-count`（优先级 1002）先于 `ip-restriction`（优先级 3000）运行，可以通过将更大的优先级数传递给 `limit-count` 的 `_meta.priority` 字段来实现：
