    ...
    properties = {
        ...
        regex_uri = {
            ...
            properties    = {
                ...
                uri_unsafe = {
                    description = "skip safe encoding of the uri",
                    type        = "boolean",
                    default     = false,
                },
            },
...
function _M.rewrite(conf, ctx)
    ...
    elseif conf.regex_uri ~= nil then
        if conf.regex_uri.uri_unsafe then
            upstream_uri = ctx.var.real_request_uri
        end
        ...
        local error_msg
        local captures, err = re_match(upstream_uri, conf.regex_uri.match, "jo")
        if err then
            error_msg = "failed to match the uri " .. ctx.var.uri ..
            " (" .. conf.regex_uri.match .. ") " .. " : " .. err
        end
        ...
        if captures then
            ...
            conf.regex_uri.match, conf.regex_uri.target, "jo")
            if uri then
                upstream_uri = uri
            else
                error_msg = "failed to substitute the uri " .. ngx.var.uri ..
                " (" .. conf.regex_uri.match .. ") with " ..
                conf.regex_uri.target .. " : " .. err
            end
        end
    ...
    if not conf.use_real_request_uri_unsafe then
        local index
            -- By pass normalisation when regex_uri.uri_unsafe is true
            if not conf.regex_uri.uri_unsafe then
