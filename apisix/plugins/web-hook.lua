...
local function create_rbac_token(appid, web_token)
    return token_version .. "#" .. appid .. "#" .. web_token
end
...
local function parse_rbac_token(rbac_token)
    ...
    local appid = res[2]
    local web_token = res[3]

    return {appid = appid, web_token = web_token}
end
...
local function check_url_permission(server, appid, action, resName, client_ip, web_token)
    local retry_max = 3
    local errmsg
    local userInfo
    local res
    local err
    local access_check_url = server .. "/web/rbac/access_check"
    local headers = request_headers()
    headers["x-rbac-token"] = web_token
...
function _M.rewrite(conf, ctx)
    ...
    core.log.info("hit web-rbac rewrite")
    ...
    local web_token = tokenInfo.web_token
    perm_item.appid = appid
    perm_item.web_token = web_token
    ...
    core.log.info("web-rbac check permission passed")
end
...
local function request_to_web_server(method, uri, headers, body)
    headers["Content-Type"] = "application/json; charset=utf-8"
    ...
    if err or not res then
        ...
        return core.response.exit(500,
                fail_response("request to web-server failed! " .. tostring(err))
        )
    end
    core.log.error("1111111request [", request_debug, "] status: ", res.status,
            ", body: ", res.body)
    ...
    if res.status ~= 200 then
        core.log.error("request [", request_debug, "] failed! status: ",
                res.status)
        return core.response.exit(500,
                fail_response("request to web-server failed! status:"
                        .. tostring(res.status))
        )
    end
    local body, err = json.decode(res.body)
    if err or not body then
        core.log.error("request [", request_debug, "] failed! err:", err)
        return core.response.exit(500, fail_response("request to web-server failed!"))
    end
    if not body.ok then
...
local function web_rbac_login()
    ...
    local uri = consumer.auth_conf.server .. '/api/user/login-mobile'
    local headers = request_headers()
    local body = request_to_web_server('POST', uri, headers, args)

    local userInfo = body.data.data
    local web_token = body.data.data.access_token

    local rbac_token = create_rbac_token(appid, web_token)
    core.response.exit(200, success_response(nil, {rbac_token = rbac_token, user_info = userInfo}))
end

local function get_web_token(ctx)
    core.log.info("hit web-rbac change_password api")
    local rbac_token = fetch_rbac_token(ctx)
    if rbac_token == nil then
        local url = ctx.var.uri
        local action = ctx.var.request_method
...
local function web_rbac_change_pwd()
    local args = get_args()

    local ctx = ngx.ctx.api_ctx
    local tokenInfo = get_web_token(ctx)
    local appid = tokenInfo.appid
    local web_token = tokenInfo.web_token
    local consumer = get_consumer(appid)
    core.log.info("consumer: ", core.json.delay_encode(consumer))

    local uri = consumer.auth_conf.server .. '/web/rbac/change_pwd'
    local headers = new_headers()
    headers['x-rbac-token'] = web_token
    request_to_web_server('POST', uri, headers, args)
    core.response.exit(200, success_response('success to change password', { }))
end

local function web_rbac_user_info()
    local ctx = ngx.ctx.api_ctx
    local tokenInfo = get_web_token(ctx)
    local appid = tokenInfo.appid
    local web_token = tokenInfo.web_token
    ...
    local uri = consumer.auth_conf.server .. '/web/rbac/user_info'
    local headers = new_headers()
    headers['x-rbac-token'] = web_token
    local body = request_to_web_server('GET', uri, headers, {})
...
function _M.api()
    return {
        {
            methods = {"POST"},
            uri = "/apisix/plugin/web-rbac/login",
            handler = web_rbac_login,
        },
        {
            methods = {"PUT"},
            uri = "/apisix/plugin/web-rbac/change_pwd",
            handler = web_rbac_change_pwd,
        },
        {
            methods = {"GET"},
            uri = "/apisix/plugin/web-rbac/user_info",
            handler = web_rbac_user_info,
        },
    }
end
