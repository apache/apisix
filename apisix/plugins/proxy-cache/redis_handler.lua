...
local redis_strategy = require("apisix.plugins.proxy-cache.redis").new
...
function _M.access(conf, ctx)
    ...
    --if not cc then
    --    core.log.warn("M:access --| : ", conf.cache_zone)
    --end
    ...
    if not ctx.cache then
        ctx.cache = {
            redis = redis_strategy({shdict_name = conf.cache_zone}),
            hit = false,
            ttl = 0,
        }
    end

    local res, err = ctx.cache.redis:get(ctx.var.upstream_cache_key)

    if ctx.var.request_method == "PURGE" then
        if err == "not found" then
            return 404
        end
        ctx.cache.redis:purge(ctx.var.upstream_cache_key)
        ctx.cache = nil
        return 200
    end
...
function _M.body_filter(conf, ctx)
    ...
    local res, err = cache.redis:set(ctx.var.upstream_cache_key, res, cache.ttl)
    if not res then
