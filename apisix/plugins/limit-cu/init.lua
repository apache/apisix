...
local plugin_name = "limit-cu"
local limit_redis_cluster_new
local limit_redis_new
do
    local redis_src = "apisix.plugins.limit-cu.limit-cu-redis"
    limit_redis_new = require(redis_src).new

    local cluster_src = "apisix.plugins.limit-cu.limit-cu-redis-cluster"
...
function _M.rate_limit(conf, ctx)
    ...
    -- Fetch the limit object
    local lim, err = gen_limit_obj(conf, ctx)

    if not lim then
        core.log.error("failed to fetch limit.cu object: ", err)
    ...
    -- Extract the compute unit (cu) from the request variables
    local cu = ctx.var["cu"]

    -- Generate the key for the incoming request
    ...
    -- Limit the incoming request based on the compute unit (cu)
    -- ! One issue with this approach is that it may result in a high complexity for the rate_limit function.
    -- ! If the value of the cu variable is large, the function will be called a large number of times,
    -- ! potentially affecting performance. In the worst case, it could even lead to exhaustion of server resources.
    -- ! Think a better way to handle this issue.
