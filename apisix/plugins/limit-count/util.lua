...
local function eval_limit(self, red, key, cost)
    local limit = self.limit
    local window = self.window
    key = self.plugin_name .. tostring(key)

    local res, err = red:eval(commit_script, 1, key, limit, window, cost)
    return res, err, window
end

function _M.redis_incoming(self, red, key, commit, cost)
    local res, err, window = eval_limit(self, red, key, commit and cost or 0)
...
function _M.redis_log_phase_incoming(self, red, key, cost)
    local res, err = eval_limit(self, red, key, cost or 1)
    if err then
        return nil, err
    end
