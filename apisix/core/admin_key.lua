...
function _M.init()
    ...
    local worker_id = ngx.worker.id() or 0  -- fallback to 0 if worker ID not available
    ...
    for attempt = 1, max_retries do
        ...
        if initialized then
            ...
            log.debug("worker ", worker_id, " loaded existing admin keys from shared memory")
            return
        end

        -- Try to acquire initialization lock
        local lock_value = worker_id .. ":" .. ngx.now() .. ":" .. attempt
        ...
        if ok then
            ...
            acquired_lock = true
            log.debug("worker ", worker_id, " acquired admin key initialization lock on attempt ", attempt)
            
            -- Triple-check initialization status after acquiring lock
            ...
            if initialized then
                ...
                _M.load_keys_from_shared_memory()
                log.debug("worker ", worker_id, " found keys already initialized after acquiring lock, released lock")
                return
            end
            ...
            if err == "exists" then
                ...
                if attempt < max_retries then
                    log.debug("worker ", worker_id, " waiting for initialization lock, attempt ", attempt)
                    ngx.sleep(retry_delay)
                else
                    -- Final attempt - just load from shared memory if available
                    log.warn("worker ", worker_id, " timeout waiting for admin key initialization lock, loading existing keys")
                    _M.load_keys_from_shared_memory()
                    return
                end
            else
                log.error("worker ", worker_id, " failed to set lock: ", err)
                return
            end
        end
    end
    ...
    if not acquired_lock then
        log.error("worker ", worker_id, " failed to acquire admin key initialization lock")
        return
    end

    log.debug("worker ", worker_id, " proceeding with admin key initialization")
    ...
    log.debug("worker ", worker_id, " completed admin key initialization and released lock")
