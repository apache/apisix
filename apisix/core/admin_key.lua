...
function _M.init()
    ...
    -- Use simple atomic check-and-set for worker coordination
    local lock_key = "init_lock"
    local worker_id = ngx.worker.id() or 0

    for attempt = 1, 50 do
        if admin_keys_shm:get("initialized") then
            _M.load_keys_from_shared_memory()
            return
        end

        local ok = admin_keys_shm:safe_set(lock_key, worker_id, 10)
        if ok then
    ...
    -- Generate keys and build complete warning message
    ...
    if has_autogenerated then
        local warning_msg = [[WARNING: using empty Admin API.
