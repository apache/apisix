...
function _M.init_worker()
    ...
    (deployment_role ~= "traditional" and deployment_role ~= "control_plane") then
        log.debug("admin_key: deployment role '", deployment_role, "' does not require admin keys, skipping initialization")
        return
    end
    ...
    if not claimed_init then
        ...
        log.debug("admin_key: another worker is handling initialization, worker ", ngx.worker.id() or 0, " will load keys lazily")
        return
    end

    -- This worker won the initialization race

    log.debug("admin_key: worker ", ngx.worker.id() or 0, " claimed initialization, processing ", #config_admin_keys, " admin keys")
