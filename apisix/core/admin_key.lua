...
function _M.init()
    ...
    -- Use atomic compare-and-swap for worker coordination
    local lock_key = "init_lock"
    local initialized_key = "initialized"
    local worker_id = ngx.worker.id() or 0

    -- Try to atomically claim initialization responsibility
    local claimed_init = admin_keys_shm:safe_set(initialized_key, "in_progress", 30)
    if not claimed_init then
        -- Another worker is initializing or already done
        for attempt = 1, 50 do
            local status = admin_keys_shm:get(initialized_key)
        ...
        _M.load_keys_from_shared_memory()
        return
    end    -- Generate keys and build complete warning message
    ...
    admin_keys_shm:set("initialized", "completed")
