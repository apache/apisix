...
function _M.init()
    ...
    -- Use a single atomic operation to claim initialization
    local worker_id = ngx.worker.id() or 0
    local lock_value = "worker_" .. worker_id .. "_" .. ngx.now()
    
    -- Only one worker can successfully set this key
    local claimed_init = admin_keys_shm:safe_set("initializing", lock_value, 30)
    if not claimed_init then
        -- Another worker is handling initialization, wait for completion
    ...
    -- Mark initialization as completed BEFORE cleaning up
    admin_keys_shm:set("completed", true)
    admin_keys_shm:delete("initializing")
